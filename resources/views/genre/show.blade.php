...
<div class="container-fluid bg-dark text-light py-5 px-5 mt-5">
    <h1 class="d-inline-flex" style="margin-right: 1000px;">{{$genre->name}}</h1>
    <form action="/genre/{{$genre->id}}" method="POST" enctype="multipart/form-data" class="d-inline-flex">
        @csrf
        @method('delete')
        <div class="d-inline-flex justify-content-start">
            <a href="{{ url()->previous() }}" class="btn btn-light mt-3 mx-2"><< Back</a>
            @if(auth()->user()->role === 'admin')
            <a href="/genre/{{$genre->id}}/edit" class="btn btn-warning mt-3">Edit</a>
...
<div class="container-fluid">
    ...
    @endforeach
    @if (!$found)
    <h3 class="d-flex justify-content-center" style="margin-top: 100px;">No books available for this genre!</h3>
    @else
